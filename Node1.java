...
public class Node1 {
  ...
  private static void rinit() {
    System.out.println("Iniciando no 1");
    initDistanceTable();
  ...
  private static void toLayer2 (Integer dest) {
    try {
      ...
      Package p = new Package(1, dest, distanceTable);
  ...
  public static void rupdate (Package p) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (distanceTable[i][1] > p.distanceTable[i][p.src] + distanceTable[p.src][1]) {
        distanceTable[i][1] = p.distanceTable[i][p.src] + distanceTable[p.src][1];
        updated = true;
