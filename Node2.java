...
public class Node2 {
  ...
  private static void rinit() {
    System.out.println("Iniciando no 2");
    initDistanceTable();
      ...
      switch (opc) {
        ...
        case 2:
          do {
            ...
            if (dest != 0 && dest != 1 && dest != 3) {
              p("Destino invalido, tente novamente!");
            }
          } while (dest != 0 && dest != 1 && dest != 3);
  ...
  private static void toLayer2 (Integer dest) {
    try {
      ...
      Package p = new Package(2, dest, distanceTable);
  ...
  public static void rupdate (Package p) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (distanceTable[i][2] > p.distanceTable[i][p.src] + distanceTable[p.src][2]) {
        distanceTable[i][2] = p.distanceTable[i][p.src] + distanceTable[p.src][2];
        updated = true;
